...
const DeleteModal = (title, isOpen, handleClose, handleDeleteRepl) => {
  return (
    <Modal isOpen={isOpen} onRequestClose={handleClose}>
      <div css={[rcss.mb(24)]}>
        <Text variant="headerDefault" css={[rcss.mb(24)]}>
          Delete Repl?
        </Text>
        <Text css={[rcss.mb(8)]}>
          Are you sure you want to delete this Repl? This cannot be undone.
        </Text>
        <Text>Some Repls may take a few minutes to finish deleting.</Text>
      </div>
      <div css={[rcss.flex.row, rcss.rowWithGap(12)]}>
        <Button text="Cancel" onClick={handleClose} />
        <Button
          iconLeft={<icons.Trash />}
          colorway="red"
          text="Yes, delete this Repl"
          onClick={() => {
            handleDeleteRepl(), handleClose();
          }}
        />
      </div>
    </Modal>
...
const ReplContainer = (data: Data) => {
  ...
  const deleteRepl = (replID: string) => {
    setIsOpen(true);
  }
          ...
          headerContent={
            ...
            >
              ...
              <IconButton
                ...
                size={28}
                onClick={() => deleteRepl(replData.repl.id)}
                disabled
              >
                <icons.Trash />
              </IconButton>
