...
const ThemeContainer = ({ themeData, sid }: any) => {
  ...
  return (
    <div className="w-full border border-[#3C445C] rounded-md p-2">
      ...
      <details>
        <summary className="p-2 my-1 btn hover:bg-[#1C2333] list-none">
          View JSON Data:
        </summary>
        <div className="rounded-md border border-[#3C445C] p-1">
          <div className="flex justify-start mb-1 gap-1">
            <button
              className="p-1 px-2 border border-[#1060a2] hover:border-[#1581d9] bg-[#1060a2] hover:bg-[#1581d9] btn"
              onClick={() => {
                gql("mutateTheme", sid, {
                  input: {
                    description: "",
                    ...
                    values:
                      JSON.parse(content).theme.currentUserInstalledThemeVersion
                        .values,
                  },
                });
              }}
